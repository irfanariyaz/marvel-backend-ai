...
async def submit_tool(data: ToolRequest, _ = Depends(key_check),  cache: CacheInterface = Depends(get_cache_service) ):     
    try: 
        # Unpack GenericRequest for tool data
        request_data = data.tool_data
        
        requested_tool = load_tool_metadata(request_data.tool_id)
        logger.info(f"requested tool:{requested_tool}")
        
        request_inputs_dict = finalize_inputs(request_data.inputs, requested_tool['inputs'])
        if (request_data.tool_id=="outline-generator" or request_data.tool_id=="slide-generator"):
            request_inputs_dict['cache']=cache
        result = await execute_tool(request_data.tool_id, request_inputs_dict)
